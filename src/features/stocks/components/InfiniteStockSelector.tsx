...
export const InfiniteStockSelector: React.FC<InfiniteStockSelectorProps> = ({
  ...
  // Load stocks
  const loadStocks = useCallback(async (page: number, search: string = '', reset: boolean = false) => {
    try {
      if (page === 1) {
        setIsLoading(true);
        setLoadError('');
  ...
  useEffect(() => {
    if (!loadMoreTriggerRef.current || !pagination.hasMore || isLoadingMore) return;
  ...
  }, [pagination.hasMore, pagination.page, isLoadingMore, searchQuery, loadStocks]);
                ...
                {pagination.hasMore && (
                  <div ref={loadMoreTriggerRef} className="p-4 text-center">
                    {isLoadingMore ? (
                      ...
                      <button
                        onClick={() => loadStocks(pagination.page + 1, searchQuery)}
