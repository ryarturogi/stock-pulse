...
export const StockCard: React.FC<StockCardProps> = ({
  ...
  const handleSaveAlert = () => {
    ...
    if (!isNaN(newPrice) && newPrice > 0) {
      console.log(
        `💾 Saving new alert price for ${symbol}: $${newPrice} (previous: $${alertPrice})`
      );
      ...
      {isLoading ? (
        ...
        <div className='space-y-1 lg:space-y-2'>
          ...
          <div className='flex items-center space-x-2'>
            <span
              className={`px-2 py-1 rounded-md text-sm font-medium ${priceChangeColor} 
          ...
          <div className='space-y-1 text-xs'>
            ...
            {lastUpdated && (
              <div className='flex items-center justify-between pt-1'>
                <div
                  className={`flex items-center ${wasRecentlyUpdated ? 'text-green-500' : 'text-gray-400 dark:text-gray-500'}`}
                >
                  <Clock
                    className={`mr-1 w-3 h-3 ${wasRecentlyUpdated ? 'text-green-500' : ''}`}
                  />
      ...
      {isLoading && (
        <div className='mt-4'>
          <div className='w-full h-1 bg-gray-200 rounded-full'>
            <div
              className='h-1 bg-blue-600 rounded-full animate-pulse'
              style={{ width: '60%' }}
            />
          </div>
        </div>
