...
export class ErrorHandler {
  ...
  static getUserMessage(error: AppError): string {
    const userMessages: Record<string, string> = {
      ...
      'API_ERROR': 'External service is temporarily unavailable. Please try again later.',
...
export const createNetworkError = (message: string, context?: Record<string, unknown>) =>
  ErrorHandler.createError(message, ErrorTypes.NETWORK_ERROR, 503, true, context);

export const createApiError = (message: string, context?: Record<string, unknown>) =>
...
export const createValidationError = (message: string, context?: Record<string, unknown>) =>
  ErrorHandler.createError(message, ErrorTypes.VALIDATION_ERROR, 400, true, context);

export const createRateLimitError = (message: string, context?: Record<string, unknown>) =>
  ErrorHandler.createError(message, ErrorTypes.RATE_LIMIT_ERROR, 429, true, context);

export const createNotFoundError = (message: string, context?: Record<string, unknown>) =>
  ErrorHandler.createError(message, ErrorTypes.NOT_FOUND_ERROR, 404, true, context);
