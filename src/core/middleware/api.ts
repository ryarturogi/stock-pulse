 ...
 * CORS middleware
 */
function handleCors(req: NextRequest, config: MiddlewareConfig): NextResponse | null {
 ...
 */
function validateMethod(req: NextRequest, config: MiddlewareConfig): NextResponse | null {
 ...
 */
function handleRateLimit(req: NextRequest, config: MiddlewareConfig): NextResponse | null {
  ...
  if (!checkRateLimit(identifier, config)) {
    ...
    const response = createErrorResponse('Rate limit exceeded', 429, 'Too many requests');
    response.headers.set('Retry-After', Math.ceil(config.rateLimitWindow! / 1000).toString());
    return response;
  }
 ...
 */
async function validateRequest(req: NextRequest, config: MiddlewareConfig): Promise<NextResponse | null> {
 ...
 */
function logResponse(req: NextRequest, response: NextResponse, startTime: number, config: MiddlewareConfig): void {
  if (!config.enableLogging) return;
 ...
 */
function addCorsHeaders(response: NextResponse, req: NextRequest, config: MiddlewareConfig): NextResponse {
