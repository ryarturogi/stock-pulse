...
test.describe('Stock Watcher Application', () => {
  ...
  test('should load the homepage successfully', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Stock Pulse/i })).toBeVisible();
  ...
  test('should add multiple stocks', async ({ page }) => {
    ...
    for (const stock of stocks) {
      ...
      await expect(page.getByTestId(`stock-symbol-${stock}`)).toContainText(stock);
    }
  ...
  test('should set price alerts', async ({ page }) => {
    await addStock(page, 'AAPL', 150.00);
    await waitForStockToLoad(page, 'AAPL');
  ...
  test('should validate alert price input', async ({ page }) => {
    ...
    // Check for error message
    await expect(page.getByTestId('error-message')).toContainText(/price must be positive/i);
  });
  ...
  test('should handle network errors gracefully', async ({ page }) => {
    ...
    await expect(page.getByTestId('error-banner')).toContainText(/network error|failed to load/i);
  ...
  test('should work on mobile devices', async ({ page, isMobile }) => {
    ...
    await stockItem.swipe({ direction: 'left' });
...
test.describe('Real-time Data Features', () => {
  ...
  test('should connect to real-time data stream', async ({ page }) => {
    ...
    // Wait for connection
    await expect(page.getByTestId('connection-status')).toContainText(/connected/i, { timeout: 10000 });
  ...
  test('should display real-time price updates', async ({ page }) => {
    ...
    await expect(priceElement).not.toContainText(initialPrice || '', { timeout: 30000 });
    
    // Verify real-time indicator is active
    await expect(page.getByTestId('real-time-indicator')).toHaveClass(/active|live/);
  });
  ...
  test('should fallback to polling when WebSocket fails', async ({ page }) => {
    ...
    await expect(page.getByTestId('connection-status')).toContainText(/polling|fallback/i);
  ...
  test('should handle WebSocket reconnection', async ({ page }) => {
    ...
    await expect(page.getByTestId('connection-status')).toContainText(/disconnected|error/i);
    
    // Restore connection
    await page.unroute('**/websocket-proxy**');
    
    // Should reconnect automatically
    await expect(page.getByTestId('connection-status')).toContainText(/connected/i, { timeout: 15000 });
  });
});
...
test.describe('Price Alerts and Notifications', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      // Mock notifications
      window.showNotificationCalls = [];
      if (navigator.serviceWorker && navigator.serviceWorker.ready) {
        navigator.serviceWorker.ready.then(registration => {
          registration.showNotification = (title, options) => {
            window.showNotificationCalls.push({ title, options });
  ...
  test('should trigger price alert notifications', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const event = new CustomEvent('price-update', {
        detail: { symbol: 'AAPL', price: 151.00 }
      });
      window.dispatchEvent(event);
    });
    
    // Check for alert notification
    const notificationCalls = await page.evaluate(() => window.showNotificationCalls);
  ...
  test('should show visual alert indicators', async ({ page }) => {
    ...
    await expect(page.getByTestId('stock-item-AAPL')).toHaveClass(/alert-active/);
  });
  ...
  test('should handle alert permission denied', async ({ page }) => {
    ...
    await expect(page.getByTestId('notification-permission-warning')).toBeVisible();
...
test.describe('Performance and Accessibility', () => {
  ...
  test('should meet accessibility standards', async ({ page }) => {
    ...
    await expect(page.getByLabelText(/stock symbol/i)).toBeVisible();
    await expect(page.getByLabelText(/alert price/i)).toBeVisible();
    
    // Check for ARIA attributes
    await expect(page.getByTestId('live-data-toggle')).toHaveAttribute('aria-label');
    await expect(page.getByTestId('connection-status')).toHaveAttribute('aria-live');
  ...
  test('should handle large numbers of stocks efficiently', async ({ page }) => {
    ...
    for (const symbol of stockSymbols.slice(0, 10)) { // Check first 10
      await expect(page.getByTestId(`stock-item-${symbol}`)).toBeVisible();
    }
...
test.describe('Cross-browser Compatibility', () => {
  test('should work consistently across browsers', async ({ page, browserName }) => {
    ...
    if (browserName !== 'webkit') { // Safari might have different WebSocket behavior
      await enableLiveData(page);
      await expect(page.getByTestId('connection-status')).toContainText(/connecting|connected|error/i);
    }
  });
});