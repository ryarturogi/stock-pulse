...
test.describe('Performance Tests', () => {
  ...
  test('should handle initial page load performance', async ({ page }) => {
    ...
    const paintMetrics = await page.evaluate(() => {
      ...
      return {
        firstPaint: entries.find(entry => entry.name === 'first-paint')
          ?.startTime,
  ...
  test('should handle large number of stocks efficiently', async ({ page }) => {
    const stockSymbols = Array.from(
      { length: 50 },
      (_, i) => `STOCK${i.toString().padStart(2, '0')}`
    );
...
test.describe('Edge Cases and Error Scenarios', () => {
  ...
  test('should handle corrupted local storage', async ({ page }) => {
    ...
    const hasCleanState = await page
      .getByText('No stocks added yet')
      .isVisible();
    const hasRecoveryMessage = await page
      .getByTestId('data-recovery-message')
      .isVisible();
...
test.describe('Stress Tests', () => {
  ...
  test('should handle maximum number of stocks', async ({ page }) => {
    ...
    const stockSymbols = Array.from(
      { length: maxStocks },
      (_, i) => `STK${i.toString().padStart(3, '0')}`
    );
