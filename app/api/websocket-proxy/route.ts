...
const activeConnections = new Map<
  string,
  { webSocket: WebSocket | null; symbols: string[] }
>();
...
export async function GET(request: NextRequest) {
  ...
  if (userLimit.blocked && now < userLimit.blockUntil) {
    ...
    console.log(
      `🚫 User rate limit: IP ${userIP} blocked for ${remainingTime}s`
    );
    return new Response(
      JSON.stringify({
        type: 'error',
  ...
  if (breaker && breaker.state === 'open') {
    ...
    if (timeSinceLastFailure < resetTimeout) {
      ...
      console.log(
        `🚫 Circuit breaker open for symbols: ${symbols}. ${remainingTime}s remaining.`
      );
      return new Response(
        JSON.stringify({
          type: 'error',
  ...
  if (cooldownTime && Date.now() < cooldownTime) {
    ...
    console.log(
      `⏰ EMERGENCY: Connection cooldown active for symbols: ${symbols}. ${remainingTime}s remaining.`
    );
    return new Response(
      JSON.stringify({
        type: 'error',
        ...
        try {
          ...
          webSocket.onerror = error => {
            ...
            if (
              ...
              console.log(
                '🚫 Rate limited by Finnhub API, setting longer cooldown'
              );
              ...
              if (breaker.failures >= 3) {
                breaker.state = 'open';
                console.log(
                  `🚫 Circuit breaker opened for symbols: ${symbols} after ${breaker.failures} failures`
                );
              }
            }
          ...
          webSocket.onclose = event => {
            ...
            const isPermanentError =
              event.code === 1002 || event.code === 1003 || event.code === 1007;
            ...
            if (reconnectAttempts >= maxReconnectAttempts || isPermanentError) {
              console.log(
                `❌ Max reconnection attempts reached (${reconnectAttempts}/${maxReconnectAttempts}) or permanent error (code: ${event.code}). Stopping reconnection.`
              );
            ...
            const backoffDelay = Math.min(
              Math.pow(3, reconnectAttempts) * baseDelay,
              180000
            ); // Cap at 3 minutes
            ...
            reconnectTimeout = setTimeout(() => {
              console.log(
                `🔄 Attempting to reconnect to Finnhub WebSocket... (attempt ${reconnectAttempts}/${maxReconnectAttempts})`
              );
              connectToFinnhub();
            }, backoffDelay);
          };
          ...
          sendEvent({
            type: 'error',
            message: 'Failed to connect to Finnhub WebSocket',
          });
        }
      };
