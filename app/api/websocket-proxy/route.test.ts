...
describe('/api/websocket-proxy', () => {
  ...
  beforeEach(async () => {
    ...
    global.TextEncoder = jest.fn().mockImplementation(() => ({
      encode: jest
        .fn()
        .mockImplementation(str => new Uint8Array(Buffer.from(str))),
    }));
  ...
  describe('Request Validation', () => {
    ...
    it('should return 500 when API key is not configured', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/websocket-proxy?symbols=AAPL'
      );
    ...
    it('should accept valid symbols parameter', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/websocket-proxy?symbols=AAPL,GOOGL'
      );
  ...
  describe('Connection Management', () => {
    it('should prevent duplicate connections for same symbols', async () => {
      ...
      expect(await response2.text()).toBe(
        'Connection already exists for these symbols'
      );
    });
    ...
    it('should allow different symbol combinations', async () => {
      ...
      const request2 = new NextRequest(
        'http://localhost:3000/api/websocket-proxy?symbols=GOOGL'
      );
  ...
  describe('Message Handling', () => {
    ...
    it('should ignore non-trade messages', async () => {
      ...
      expect(console.log).not.toHaveBeenCalledWith(
        expect.stringContaining('💰 Real-time trade:')
      );
    });
  });
  ...
  describe('Reconnection Logic', () => {
    it('should attempt reconnection on WebSocket close', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining(
          '🔄 Attempting to reconnect to Finnhub WebSocket'
        )
      );
    });
    ...
    it('should implement exponential backoff for reconnections', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining(
          '🔄 Attempting to reconnect to Finnhub WebSocket... (attempt 3/5)'
        )
      );
    });
  ...
  describe('Cleanup and Error Handling', () => {
    ...
    it('should clean up on client disconnect', async () => {
      ...
      const cleanupCallback =
        mockAbortController.signal.addEventListener.mock.calls[0][1];
      cleanupCallback();
  ...
  describe('Symbol Processing', () => {
    it('should handle single symbol', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('📈 Processing symbols:', [
        'AAPL',
      ]);
    });
    ...
    it('should handle multiple symbols', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/websocket-proxy?symbols=AAPL,GOOGL,MSFT'
      );
    ...
    it('should handle symbols with whitespace', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/websocket-proxy?symbols=AAPL,%20GOOGL%20,MSFT'
      );
    ...
    it('should handle URL encoded symbols', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/websocket-proxy?symbols=BRK%2EA,BRK%2EB'
      );
  ...
  describe('Rate Limiting and Performance', () => {
    ...
    it('should handle many symbols efficiently', async () => {
      ...
      const request = new NextRequest(
        `http://localhost:3000/api/websocket-proxy?symbols=${symbols}`
      );
      ...
      expect(console.log).toHaveBeenCalledWith(
        '📈 Processing symbols:',
        expect.arrayContaining(['STOCK0', 'STOCK49'])
      );
    });
  });
  ...
  describe('Connection Pool Edge Cases', () => {
    ...
    it('should handle empty symbols parameter edge case', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/websocket-proxy?symbols='
      );
