...
export default function HomePage() {
  ...
  const {
    ...
    filteredItems: filteredWatchedStocks,
  } = useSearch(
    watchedStocks,
    (stock, query) =>
  ...
  useEffect(() => {
    ...
    if (watchedStocks.length > 0 && isLiveDataEnabled) {
      ...
      if (
        ...
        console.log(
          '🔌 Initializing WebSocket connection for real-time data...'
        );
  ...
  const handleAddStock = async (
    ...
    if (!stock && !stockName) {
      try {
        ...
        console.warn(
          `Could not fetch stock info for ${symbol}, using symbol as name:`,
          error
        );
        finalStockName = symbol;
      }
      ...
      if (watchedStocks.length === 0) {
        try {
          ...
          if (permission === 'granted') {
            console.log(
              '✅ Webpush notification permission granted! You will receive price alerts.'
            );
            ...
            console.log(
              '⏳ Notification permission dismissed. You can enable it later.'
            );
          }
        } catch (error) {
      ...
      setTimeout(async () => {
        try {
          ...
          if (quoteData && quoteData.current) {
            ...
            console.log(
              `💰 Initial data loaded for ${symbol}: $${quoteData.current}`
            );
          }
        } catch (error) {
    ...
    try {
      ...
      const successful = results.filter((result, index) => {
        ...
        if (result.status === 'rejected') {
          console.error(
            `Refresh failed for ${watchedStocks[index]?.symbol || 'unknown'}:`,
            result.reason
          );
        }
        return false;
      ...
      console.log(
        `📊 Manual refresh completed: ${successful.length}/${watchedStocks.length} stocks updated`
      );
